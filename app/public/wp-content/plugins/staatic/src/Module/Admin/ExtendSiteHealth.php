...
final class ExtendSiteHealth implements ModuleInterface
{
    ...
    public function addDebugInformation(array $info) : array
    {
        $htmlDomParsers = [
            'html5' => 'HTML5-PHP',
        ...
        $htmlDomParser = \get_option('staatic_crawler_dom_parser', null);
        $processNotFound = \get_option('staatic_crawler_process_not_found', null);
        $deploymentMethod = \get_option('staatic_deployment_method', null);
        $downgradeHttps = \get_option('staatic_http_https_to_http', null);
        $sslVerifyBehavior = \get_option('staatic_ssl_verify_behavior', null);
        $sslVerifyPath = \get_option('staatic_ssl_verify_path', null);
        $testRequestStatus = \get_option(TestRequest::OPTION_NAME, null);
        $info['staatic'] = [
            ...
            'fields' => [
                ...
                'destination_url' => [
                    ...
                    'value' => \get_option('staatic_destination_url', null) ?? \__('Undefined', 'staatic')
                ],
                'deployment_method' => [
                ...
                'http_auth_username' => [
                    ...
                    'value' => \get_option('staatic_http_auth_username', null) ?? \__('Undefined', 'staatic')
                ],
                'http_concurrency' => [
                    'label' => \__('HTTP concurrency', 'staatic'),
                    'value' => \get_option('staatic_http_concurrency', null) ?? \__('Undefined', 'staatic')
                ],
                'http_https_to_http' => [
                ...
                'http_timeout' => [
                    ...
                    'value' => \get_option('staatic_http_timeout', null) ?? \__('Undefined', 'staatic')
                ],
                'ssl_verify_behavior' => [
                ...
                'publication_task_timeout' => [
                    'label' => \__('Publication task timeout', 'staatic'),
                    'value' => \get_option('staatic_background_process_timeout', null) ?? \__('Undefined', 'staatic')
                ],
                'test_request_status' => [
